...
func (c *Customer) generateRandomNumberOfProducts() {
	num := rand.Float64()
	if num >= 0.2 {
		c.noOfProducts = rand.Intn(194) + 6
		test := rand.Intn(100)
		if test >= 99 {
			c.noOfProducts = rand.Intn(194) + 6
		} else if num >= 98 {
			c.noOfProducts = rand.Intn(174) + 6
		} else if num >= 97 {
			c.noOfProducts = rand.Intn(154) + 6
		} else if num >= 95 {
			c.noOfProducts = rand.Intn(134) + 6
		} else if num >= 90 {
			c.noOfProducts = rand.Intn(114) + 6
		} else if num >= 50 {
			c.noOfProducts = rand.Intn(70) + 6
		} else {
			c.noOfProducts = rand.Intn(20) + 6
		}
	} else {
		c.noOfProducts = rand.Intn(4) + 1
	}
}
...
func InputNumberOfCheckoutsAndConstumers() {
	fmt.Println("As a manager, enter the number of checkouts, Max 8")
	fmt.Scanln(&totalCheckouts)
	fmt.Println("Enter the number of Express Checkouts , Max 2")
