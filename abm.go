...
var (
	...
	// in a checkout, the waiting time for customer3 will be the waiting time for customer2 + the waiting time for customer3
...
func producer(checkoutNo int,ch (chan customer)){
		...
		for j:=0;j<cusNum;j++ {
			...
			if isCheckoutRestricted[checkoutNo] {
				...
				//1 to 10 for testing
				newCustomer.trolleyItem = GenerateRangeNum(1,10)
			}
...
func initArrivingTimeToCheckout() int{
	...
	switch Cloudy {
	case Cloudy:return GenerateRangeNum(0,5)
...
func checkout(checkoutNo int,ch (chan customer),timeForProduct float32) {
	for {
		customer := <-ch
		waitingTime := (float32)(customer.trolleyItem)*timeForProduct
